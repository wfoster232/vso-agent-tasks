...
foreach ($clean in @($true, $false)) {
    foreach ($restoreNuGetPackages in @($true, $false)) {
        foreach ($logProjectEvents in @($true, $false)) {
            foreach ($requireMSBuildVersion in @($true, $false)) {
                Unregister-Mock Select-MSBuildLocation
                Unregister-Mock Invoke-BuildTools
                Register-Mock Select-MSBuildLocation { 'Some location' } -- -Method 'Some input method' -Location 'Some input location' -Version 'Some input version' -RequireVersion: $requireMSBuildVersion -Architecture 'Some input architecture'
                ...
                $splat = @{
                    ...
                    'RequireMSBuildVersion' = $requireMSBuildVersion.ToString()
